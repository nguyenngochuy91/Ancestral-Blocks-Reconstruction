    ...
    for fname in fname_list:
        for line in [i.strip() for i in open(fname).readlines()]:
            ...
            try: # faster implementation than "if predicted_gene in gene_block_dict.keys():"
                ...
                #if gene_block == 'casABCDE12':
                #    print 'AFDFAFDSF'
                if gene_block in list(result.keys()):    # check if the gene block is in the result dict already, if not make a new entry in the else clause
                    if accession in list(result[gene_block].keys()): # Check if the organism has been added to the gene block
